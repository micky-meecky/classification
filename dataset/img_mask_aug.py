...
def data_aug(imgs, masks, contour, dist, segs=None):     # 输入图像和标签,输入输出格式为numpy
    ...
    contours = np.array(contour).astype(np.uint8)
    ...
    if batch_num > 1:   # 多batch处理
        ...
        contours_aug = np.zeros(contours.shape)
        distances_aug = np.zeros(dists.shape)
        ...
        for i in range(0, batch_num):
            ...
            segcontour = ia.SegmentationMapsOnImage(contours[i, :, :], shape=contours[i, :, :].shape)  # 分割标签格式
            contours_aug[i, :, :] = seq_det.augment_segmentation_maps(segcontour).get_arr().astype(np.uint8)   # 将方法应用在分割标签上，并且转换成np类型
            segdist = ia.SegmentationMapsOnImage(dists[i, :, :], shape=dists[i, :, :].shape)  # 分割标签格式
            distances_aug[i, :, :] = seq_det.augment_segmentation_maps(segdist).get_arr().astype(np.uint8)   # 将方法应用在分割标签上，并且转换成np类型
        ...
        segcontour = ia.SegmentationMapsOnImage(contours, shape=contours.shape)  # 分割标签格式
        contours_aug = seq_det.augment_segmentation_maps(segcontour).get_arr().astype(np.uint8)  # 将方法应用在分割标签上，并且转换成np类型
        segdist = ia.SegmentationMapsOnImage(dists, shape=dists.shape)  # 分割标签格式
        distances_aug = seq_det.augment_segmentation_maps(segdist).get_arr().astype(np.uint8)  # 将方法应用在分割标签上，并且转换成np类型
    ...
    if  segs is not None:
        ...
        return images_aug, segmaps_aug, contours_aug, distances_aug
